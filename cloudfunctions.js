const abi = [
  ...
  },
  {
    ...
    outputs: [ { internalType: 'contract BankersDreamDividendTracker', name: '', type: 'address' } ],
    stateMutability: 'view',
    type: 'function'
  },
  {
  ...
  {
    inputs: [ { internalType: 'uint256', name: '_buybackFee', type: 'uint256' } ],
    name: 'setBuybackFee',
    outputs: [],
    stateMutability: 'nonpayable',
    type: 'function'
  ...
  {
    inputs: [ { internalType: 'uint256', name: '_marketingFee', type: 'uint256' } ],
...
Moralis.Cloud.define('getTotalDistributed', async (request) => {
  const web3 = Moralis.web3ByChain('0x38'); // mainnet
  const contract = new web3.eth.Contract(abi, '0x966f75a3A48BD6133220Bf83A62429bf04Adf29f');
...
Moralis.Cloud.define('getShares', async (request) => {
  const web3 = Moralis.web3ByChain('0x38'); // mainnet
  const contract = new web3.eth.Contract(abi, '0x966f75a3A48BD6133220Bf83A62429bf04Adf29f');
  const paid = await contract.methods
    .getAccountDividendsInfo(request.params.address)
    .call()
    .catch(() => 'error with code');
  const unpaid = await contract.methods
    .withdrawableDividendOf(request.params.address)
    .call()
    .catch(() => 'error with code');
  const options = {
    address: '0x3ee2200efb3400fabb9aacf31297cbdd1d435d47',
